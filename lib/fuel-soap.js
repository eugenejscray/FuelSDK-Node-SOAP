...
FuelSoap.prototype.retrieve = function(type, props, options, callback) {
	...
	if(options.ContinueRequest != undefined){
		
		// We can retrieve more results from the previous request id.
		// We do not need a filter if we pass in ContinueRequest
		body.RetrieveRequestMsg.RetrieveRequest = {};
		body.RetrieveRequestMsg.RetrieveRequest.ContinueRequest = options.ContinueRequest;
	}
	else if(filter){
...
FuelSoap.prototype._parseResponse = function(key, body, callback) {
	...
	parseString(body, parseOptions, function(err, res) {
		...
		console.log(parsedRes.RequestID);
